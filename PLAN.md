...
# [Stage 14.4] Cross-Platform Payment Flow Audit & Fix

## Objective
CEO 가 데스크탑에서 여러 차례 테스트했지만 결제창이 한 번도 뜨지 않았다.
- 모바일 텔레그램: `requestStarsInvoice → openInvoice` 네이티브 결제 시트
- 데스크탑 텔레그램: `requestStarsInvoice → openTelegramLink(invoiceLink)` 결제 대화창
- 일반 브라우저: `requestStarsInvoice → window.open(invoiceLink)` 새 탭
이 의도된 시나리오 중 데스크탑/일반 브라우저 경로가 깨져 있던 원인을 종합 진단·해결.

## Root Cause Diagnosis
1. **`window.open` popup blocker 차단** — `await requestStarsInvoice()` 다음에 호출되는 `window.open()` 은 user-gesture 컨텍스트를 잃어 Chrome/Firefox/Safari 가 모두 차단. 일반 브라우저 테스트에서 100% 막힘.
2. **silent fail 다수** — `if (!telegramUserId) return`, `if (res.ok && res.invoiceLink)` else 부재, `openInvoice` 콜백의 `cancelled`/`failed`/`pending` 처리 부재. CEO 가 무엇이 실패했는지 알 길이 없었음.
3. **분기 추적 불가** — `console.log` 한 줄도 없어 어느 경로(native/tg-link/redirect)로 들어갔는지 사후 진단 불가능.

## Atomic Tasks

### Task 1 — `openStarsInvoice` 통합 헬퍼
- `web/src/utils/telegram.ts` 에 `openStarsInvoice(invoiceLink, onPaid, onError)` 추가.
- 우선순위: `tg.openInvoice` → `tg.openTelegramLink` → `window.location.href`.
- `window.open` 사용 금지 (popup blocker).
- 모든 분기에 `console.log('[Payment] ...')`. `openInvoice` 콜백의 4가지 status 명시 처리.
- 반환값 `'native' | 'tg-link' | 'browser-redirect'` 로 호출자가 후속 state 분기 가능.

### Task 2 — 3개 결제 핸들러 헬퍼 호출로 통일
- `PremiumTab.tsx` Elite 업그레이드, `TradeTab.tsx` `handleRecharge`, `PortfolioTab.tsx` `handleRecharge` 모두 헬퍼 호출 한 줄로 교체.
- `!telegramUserId` 가드는 Toast/Error 로 사용자 피드백.
- `!invoiceLink` 도 명시 분기로 사용자 피드백.
- catch 블록에 `console.error('[Payment] ...:')` 추가.
- 반환값이 `'native'` 면 콜백에서 `setIsPaying`/`setStarsPending` 끄기, 그 외엔 즉시 끔.

## Negative Constraints
- 백엔드(`bot/**`) 코드 변경 금지 — `/api/payment/stars` 엔드포인트는 Stage 14.2 그대로.
- `openTelegramLinkSafe` 삭제 금지 (referral 등에서 추후 재사용 가능).
- VIPTab, ShareROI, Referral copy 등 결제 외 흐름 손대지 않음 (최소 diff).
- `.env.production` 변경 없음.

## Verification
- `tsc --noEmit` 통과 (web).
- 모바일 텔레그램 → `[Payment] openInvoice available: true` 로그 + 네이티브 시트.
- 데스크탑 텔레그램 → `[Payment] openInvoice available: true|false` 확인 후 적절 분기.
- 일반 브라우저 → `location.href` redirect 로 텔레그램 deep link 활성, popup blocker 무관.
- 모든 silent fail 제거 — 어떤 실패도 Toast/Error UI 또는 console 로 가시화.
