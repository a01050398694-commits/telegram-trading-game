...
- **MUST** keep `drawdownBrake.lastTriggerLossExitAt` idempotency check. The brake reconciles every 10 min, so without it the same N stale losses re-extend `cooldownUntil` to "now + 4h" forever. After 2026-05-06 the 5 broken pre-Stage-22 BTC signals all hit SL → reconcile saw 5 losses every cycle → cooldown never expired even after Stage 22.1 SMA200 fix landed.
...
- **MUST NOT** rerun a known-broken signal cohort through performance views or risk circuits. Pre-Stage-22 signals with TP < entry must be reclassified `status='invalid'` (migration 16 backfill) so `v_signal_performance_30d` stops counting them in `total_closed` and `drawdownBrake.reconcileFromDB()` (which queries `status='closed'`) stops seeing them as recent losses.
...
## Stage 22 — Signal pipeline correctness (2026-05-09)

### MUST
- **MUST** use signed R:R deltas, not `Math.abs(tp - entry)`, in any signal-validation gate. Pre-Stage-22 the abs-form let LONG-with-TP-below-entry pass with a positive rr value; the mistake recorded "tp2 hit" labels with negative pnlR for 5 of 8 broadcasts on 2026-05-06. Long: `(tp - entry) / slDist`. Short: `(entry - tp) / slDist`. Negative = malformed → reject.
- **MUST** treat `findNearestSupportResistance` returning `null` as a legitimate "breakout territory, defer entry" condition. The pre-Stage-22 fallback fabricated a level on the wrong side of price — that was the literal root cause of the 2026-05-06 incident. Engine must skip with a rationale, not invent a level.
- **MUST** drop the in-progress live candle BEFORE computing TA in the live path (`signalCron` calls `marketData.dropInProgress()` once per TF). Backtest passes only-closed bars and skips the drop. signalEngine itself assumes input is closed-only — doing the drop inside buildSignal silently destroys backtest data and produced 0 entries on iter 1.
- **MUST** anchor the dedup `setup_hash` to STRUCTURE (`recentSwingHigh` + `recentSwingLow`), not to live entry/tp prices. Price-based hashes fragment with tick noise (a 25-bp bucket at $80k = $200 = one candle's range), letting near-identical setups slip through. Structure changes only when a new swing forms — exactly when a new setup deserves to broadcast.
- **MUST** record EVERY tick outcome to `signal_outcomes` (status ∈ `open`/`closed`/`skipped`/`deduped`/`invalid`). 96% of pre-Stage-22 ticks were invisible because only broadcasts hit the DB; operators couldn't tell "healthy filter" from "cron dead". The `v_signal_performance_30d` view depends on the full audit trail.
- **MUST** read the honest performance from `v_signal_performance_30d` (filters by `pnl_r_net > 0`, not by the `hit` label). The `/stats` admin command is the single boundary where users see the real number — never re-derive win rate from `hit` outside this view.
- **MUST** apply `FEE_R_DEDUCTION = 0.13` to backtest outcomes BEFORE computing stats. signalOutcome.closeOutcome subtracts it on live close; if backtest doesn't, gross/net diverge and the deploy decision is wrong.
- **MUST** keep `MIN_MTF_CONFLUENCE = 3` (out of 4: D1 trend, H4 trend, H1 momentum, volume). Iter2 loosened to 2 to chase cadence and produced -27.96R / PF 0.53 in 60d backtest. Quality > quantity — 0.2 signals/day at PF 2+ beats 1.4 signals/day at PF 0.5.
- **MUST** keep validator gate ordering: G1 (TP direction) BEFORE G2 (R:R). G1's check is structural (TP on profitable side), G2's is quantitative (rr ≥ 1.0). Reversing means G2 might emit a confusing rr-floor message when the real bug is direction.

### MUST NOT
- **MUST NOT** resurrect `COOLDOWN_MS = 0` / `SKIP_COOLDOWN_MS = 0` defaults. These existed for the broken pre-Stage-22 era when signals were "free" — now each signal incurs DB rows and channel post. Keep cooldowns 60min/15min plus the 6h structural dedup hash plus the boot-tick gate (DB last_broadcast < 30min).
- **MUST NOT** allow `simulateTrade` to run on a malformed signal silently. The defense-in-depth assertion (`tp1 > entry` for long) throws a runtime error if validator was bypassed. Catch upstream and mark the row as `status='invalid'` so the failure is visible, not absorbed.
- **MUST NOT** enforce small-sample (`n<30`) Sortino / Calmar / win-rate-upper gates as deploy blockers. With n=12 these ratios are statistically unstable. The mechanical acceptance gate marks them `n/a` and gates only on PF, expectancy, max-DD, loss-streak, win-rate-lower, cadence — those are statistically robust at small n.
- **MUST NOT** re-introduce a fabricating `findNearestSupportResistance` fallback. If price has rallied past all swings, that's a *real* condition (clean breakout) requiring a real response (skip + rationale). Inventing a level is what produced the 2026-05-06 disaster.
- **MUST NOT** trust the `hit` field as a proxy for "win". The simulator's `hit='tp2'` only means "price reached the tp2 level", which can be a LOSS if the signal was malformed. Derive wins from `pnl_r_net > 0` everywhere — `/stats`, `/dailyReport`, public-facing metrics.
