# Telegram Trading Game

텔레그램 기반 실시간 가상 트레이딩 경쟁 게임. 작업 규칙은 `CLAUDE.md`, 진행 현황은 `PROGRESS.md` 참고.

## Structure
```
telegram-trading-game/
├── web/            # Telegram Mini App (Vite + React 19 + TS + Tailwind v4)
├── bot/            # Telegram Bot + API server (Node + TS + grammY + Express)
├── supabase/       # DB schema migrations
├── docs/           # Design tokens, ops runbooks
├── CLAUDE.md       # Coding/communication rules
├── PROGRESS.md     # Session progress log
└── GOTCHAS.md      # Accumulated MUST/MUST NOT rules
```

## Deployment
- **Telegram Mini App**: Vercel (auto-deploy on `main` push, `vercel.json`)
- **Telegram Bot**: Render (auto-deploy on `main` push, `render.yaml`)
- **Database**: Supabase (PostgreSQL 17 + RLS)
- **Payments**: InviteMember (PayPal + Telegram Stars, 8 plans)

## Setup
1. `npm install` (root에서 실행, workspaces가 web/bot 동시 설치)
2. `.env.example` → `.env` 복사, 필수 키 입력
3. `npm run dev` — web(5173) + bot(3000) 동시 기동

## Scripts
| 명령 | 설명 |
|---|---|
| `npm run dev` | web + bot 동시 실행 |
| `npm run dev:web` | web 단독 |
| `npm run dev:bot` | bot 단독 |
| `npm run typecheck` | 전체 TS 타입 검사 |
| `npm run build` | 프로덕션 빌드 |
| `npm test -w bot` | 봇 단위 테스트 (vitest) |
