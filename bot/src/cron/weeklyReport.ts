...
export class WeeklyReportCron {
  ...
  async sendAllReports(): Promise<{ sent: number; errors: number }> {
    ...
    try {
      ...
      for (const user of premiumUsers as { id: string; telegram_id: number }[]) {
        try {
          const report = await computeWeeklyReport(this.db, user.id, user.telegram_id);
          await this.sendReportDM(report);
          sent++;
        } catch (err) {
  ...
  private async sendReportDM(report: Awaited<ReturnType<typeof computeWeeklyReport>>): Promise<void> {
    ...
    await this.bot.api.sendMessage(report.telegramId, message, {
      ...
      reply_markup: {
        inline_keyboard: [[
          { text: '📈 자세히 보기', web_app: { url: webAppUrl() } },
        ]],
      },
    });
  }
